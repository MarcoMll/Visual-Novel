...
namespace CustomInspector.Documentation
{
    ...
    /// </summary>
    //[CreateAssetMenu(menuName = "CustomInspector/SnippetsReader")]
    public class SnippetsReader : ScriptableObject
    {
        ...
        public void DrawDescription(Rect position, NewPropertyD drawer)
        {
            ...
            GUIStyle style = new();
            style.normal.textColor = FixedColorConvert.ToColor(FixedColor.DarkGray);
            style.wordWrap = true;

            Rect contentRect = new(0, 0, position.width - scollbarWidth, style.CalcHeight(content, position.width));
        ...
        public void DrawPreview(Rect position, NewPropertyD drawer)
        {
            ...
            if (previewObj == null)
            {
                //ScriptableObject cs = ScriptableObject.CreateInstance(typeof(CodeSnippets));
                previewObj = new();
                //Call start methods
                var fields = typeof(CodeSnippets).GetFields(PropertyValues.defaultBindingFlags);
                foreach (var field in fields)
                {
                    if (PropertyValues.ContainsMethod(field.GetValue(previewObj), "Start", out (MethodInfo methodInfo, object owner) method))
                    {
                        try
                        {
            ...
            //Draw Propertys
            var props = container.GetAllVisiblePropertys();
            float totalHeight = props.Select(_ => EditorGUI.GetPropertyHeight(_, new GUIContent(_.name, _.tooltip))).Sum() + 30;
                                                    ...
                                                    previewRect))
            {
                ...
                foreach (SerializedProperty prop in props)
                {
                    ...
                    Rect lineRect = new(x: 0,
                                        y: height,
                                        width: previewRect.width,
                                        height: EditorGUI.GetPropertyHeight(prop, label, true));

...
namespace CustomInspector
{
    ...
    public class AssetPathDrawer : PropertyDrawer
    {
        ...
        float fileOpenDialogButtonSize => EditorGUIUtility.singleLineHeight;
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            Type requiredType = (Type)property.GetDirtyValue("fileType",
                    BindingFlags.FlattenHierarchy | BindingFlags.Instance | BindingFlags.NonPublic);

            if(requiredType == null) //noone called = new() | so unity used some sort of copy to not call the constructor
            {
            ...
            Debug.Assert(requiredType != null, "fileType not found");

            //new content showing type
            ...
            using (new FixedIndentLevel(0))
            {
                ...
                if (!isValid)
                {
                    float xStart = position.x //label start
                                 + Mathf.Min(EditorGUIUtility.labelWidth, GUI.skin.label.CalcSize(label).x) //label content width
            ...
            //Draw label
                ...
                {
                    width = position.width * (pathToObjectRatio),
                };

...
{
    /// <summary>
    /// Only valid for SerializableSortedSetAttribute! Used to for display in the inspector
    /// </summary>
    [Obsolete("Use the " + nameof(SetAttribute) + " instead")]
    public class SerializableSortedSetAttribute : PropertyAttribute { }
    ...
    public class SerializableSortedSet<T> : SerializableSet<T>, ICollection, ICollection<T>, IEnumerable, IEnumerable<T> where T : IComparable
    {
        ...
        (int index, bool isSame) GetTheoreticalIndex(T item)
        {
            ...
            {
                switch (values[0].CompareTo(item))
                {
                    case -1: //list[0] smaller
                        return (1, false);
                    case 0: //list[0] equal
                        return (0, true);
                    case 1: //list[0] greater
                        return (0, false);
                    default:
                        throw new ArgumentOutOfRangeException("Unexpected value from CompareTo");
                }
            }
            else return (0, false);
            ...
            (int index, bool isSame) FindItemBetween(int min, int max)
            {
                ...
                if (min == max) //Abbruch bedingung - ab hier ist die mitte von beiden ja eh nur das niedrigere
                {
                    ...
                    {
                        -1 => (min + 1, false), //min smaller
                        0 => (min, true), //min equal
                        1 => (min, false), //min greater
                        _ => throw new ArgumentOutOfRangeException("CompareTo returns unexpected value"),
                    };
                }
                if (min + 1 == max) //Abbruch bedingung - ab hier ist die mitte von beiden ja eh nur das niedrigere
                {
                    ...
                    {
                        case -1: //list[min] smaller
                            return values[max].CompareTo(item) switch
                            {
                                -1 => (max + 1, false), //max smaller
                                0 => (max, true), //max equal
                                1 => (max, false), //max greater
                                _ => throw new ArgumentOutOfRangeException("CompareTo returns unexpected value"),
                            };
                        case 0: //list[min] equal
                            return (min, true);
                        default: //list[min] greater
                            return (min, false);
                    }
                }
                ...
                {
                    -1 => FindItemBetween(middle + 1, max), // list[middle] < item
                    0 => (middle, true), // list[middle] == item
                    1 => FindItemBetween(min, middle - 1), // list[middle] > item
                    _ => throw new ArgumentOutOfRangeException("CompareTo returns unexpected value"),
                };
            }
        }
